...
describe('State Machine', () => {
  ...
  it('should create a state machine with lambda functions', async () => {
    ...
    expect(synthesized).toHaveResourceWithProperties(SfnStateMachine, {
      definition:
        '{"StartAt":"task1","States":{"task2":{"Type":"Task","Resource":"arn:aws:states:::lambda:invoke","Arguments":{"Payload":"{% $state.input.data %}","FunctionName":"test-function"},"End":true,"Output":"{% $exists($states.result.Payload) ? $states.result.Payload : {} %}"},"task1":{"Type":"Task","Resource":"arn:aws:states:::lambda:invoke","Arguments":{"Payload":{"executionId":"{% $states.context.Execution.Id %}"},"FunctionName":"test-function"},"Next":"task2","Assign":{"foo":1},"Output":"{% $exists($states.result.Payload) ? $states.result.Payload : {} %}"}},"QueryLanguage":"JSONata"}',
      name: 'TestingSM',
    });
  });
  ...
  it('should create state machine with integration', async () => {
    ...
    expect(synthesized).toHaveResourceWithProperties(SfnStateMachine, {
      definition:
        '{"StartAt":"integration","States":{"integration":{"Type":"Task","Resource":"arn:aws:states:::sqs:sendMessage.waitForTaskToken","Arguments":{"QueueUrl":"${aws_sqs_queue.test.id}","MessageBody":{"Message":"test","TaskToken":"{% $states.context.Task.Token %}"}}}},"QueryLanguage":"JSONata"}',
      name: 'TestingSM',
    });
  });
