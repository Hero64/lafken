---
title: "@lafken/common"
description: "@lafken/common is the core utility package for the Lafken framework. It provides the decorator factory functions, metadata reflection utilities, and shared types that power every @lafken/* package. If a Lafken package defines a decorator or reads infrastructure metadata, it depends on @lafken/common."
---
...
A method decorator that marks a Lambda handler as a **response-streaming** handler (`awslambda.streamifyResponse`). Combine it with a handler built from `createLambdaDecorator` — `@Handler` in `@lafken/standalone`, or `@Get`/`@Post`/etc. in `@lafken/api` with `integrationType: 'aws-proxy'`:

```typescript
import { Context, Streaming, ResponseStreaming } from '@lafken/common';

class ChatService {
  @Streaming()
  @Handler()
  stream(@ResponseStreaming() responseStream: Writable, @Context() ctx: any) {
...
`@Streaming()` only records, at build time, which methods stream a response — it does not change the handler's behavior directly. The actual `streamifyResponse` wrapping happens when the handler is exported as the Lambda entry point.

Because of that, the decorated method's runtime signature changes from `(event, context)` to `(event, responseStream, context)`. `@ResponseStreaming()` injects the stream argument; `@Context()` keeps working exactly as before, now reading from the third argument instead of the second.

:::note
The distinction between the two signatures is structural, not `instanceof`-based: the framework checks whether the second argument exposes a `write` function. This matters because the Lambda runtime hands over its own stream implementation, which does not necessarily inherit from Node's `stream` module.
:::

Only the default Lambda integration can invoke a streaming handler — other integrations (`bucket`, `queue`, `mock`, ...) never invoke a Lambda directly, so `@Streaming()` has no effect there. See [`@lafken/api`'s Lambda proxy integration](/packages/api/#lambda-proxy-integration) for the concrete case this exists for: streaming an HTTP response through API Gateway.
