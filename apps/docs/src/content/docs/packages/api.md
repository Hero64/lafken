---
title: "@lafken/api"
description: "Build AWS REST APIs using TypeScript decorators. @lafken/api lets you declare endpoints, request/response models, authorizers, and AWS service integrations directly in your classes — Lafken generates all the API Gateway and Lambda infrastructure for you."
---
...
The `definition` option on `restApi` controls **how API Gateway generates and manages your API structure**. It accepts two values:

| Value      | Default | Description                                                                                           |
| ---------- | ------- | ----------------------------------------------------------------------------------------------------- |
| `resource` | ✅      | Each element of the API (resources, methods, integrations, models, validators, authorizers, docs) is created as an individual Terraform resource. |
| `openapi`  |         | The whole API structure is assembled into a single OpenAPI 3.0 document (with `x-amazon-apigateway-*` extensions) and imported into the REST API `body`. |
...
Each API Gateway element becomes its own Terraform resource (`aws_api_gateway_resource`, `aws_api_gateway_method`, `aws_api_gateway_integration`, `aws_api_gateway_model`, `aws_api_gateway_request_validator`, `aws_api_gateway_authorizer`, documentation parts, and so on).

- **Pros**: granular and debuggable — Terraform reports exactly which resource changed, and each element can be inspected independently in the AWS console.
- **Cons**: on large APIs, deployment is slower because Terraform diffs and verifies every resource on each deploy.

#### `openapi` mode

All paths, methods, integrations, schemas, request validators, authorizers, gateway responses, and documentation parts are accumulated into a single OpenAPI 3.0 document and injected into the REST API `body`. API Gateway imports the whole API in one shot.

- **Pros**: significantly faster deploys on large APIs — a single `body` update replaces dozens of individual resource diffs.
- **Cons**: less granular — the API structure is managed as one blob, so Terraform diffs it as a whole.

Side resources that the body only references are **still created as regular Terraform resources** in both modes: Lambda functions and permissions, IAM roles, usage plans and API keys, gateway responses, log groups, deployments, and stages.

#### Documentation in the exported spec

Both modes support the same decorator-driven documentation — `tags` and `description` on `@Api`, and `summary`, `description`, and `tags` on method decorators:
...
export class ArticleApi {
  @Get({
    ...
    description: 'Get the full article details',
  })
  getById() { /* ... */ }
}
```

In `openapi` mode this documentation is emitted inline in the OpenAPI document (`tags`, `summary`, `description` fields) and included in any OAS3 export from the deployed API — whether generated by `aws apigateway get-export` or the AWS console. To make the export include the full documentation, the stage references a documentation version that Lafken creates automatically.

In `resource` mode the same information is stored as `aws_api_gateway_documentation_part` resources and an associated documentation version.
...
By default (`integrationType: 'aws'`), API Gateway maps the request into the shape declared via `@Event(...)` using a VTL request template before invoking the Lambda, and maps the Lambda result back through the [response templates](#responses).

Set `integrationType: 'aws-proxy'` to switch to a **proxy integration** instead: API Gateway forwards the raw `APIGatewayProxyEvent` to the Lambda and returns its response verbatim, with no request or response templates involved.

```typescript
import { Api, Post, EventProxy } from '@lafken/api/main';
import type { APIGatewayProxyEvent } from 'aws-lambda';

@Api({ path: '/webhooks' })
class WebhookApi {
  @Post({ integrationType: 'aws-proxy' })
  handle(@EventProxy() event: APIGatewayProxyEvent) {
...
`@EventProxy(...)` replaces `@Event(...)` as the parameter decorator when `integrationType: 'aws-proxy'` is set — using it with the default `'aws'` integration fails at synth. It optionally accepts a request class, exactly like `@Event`, used only to generate the **input model** (OpenAPI request-body schema and API Gateway request validation); it does not change the raw event delivered to the handler.

`integrationType: 'aws-proxy'` is also **required** for response streaming — combine it with `@Streaming()` and `@ResponseStreaming()` from [`@lafken/common`](/packages/common/#streaming):
...
class ChatApi {
  ...
  @Post({ integrationType: 'aws-proxy' })
  stream(
    @EventProxy() event: APIGatewayProxyEvent,
