---
title: "@lafken/dynamo"
description: "Define and manage DynamoDB tables using TypeScript decorators. @lafken/dynamo lets you declare table schemas, indexes, streams, and TTL directly in your classes — and provides a type-safe repository for performing operations at runtime."
---
...
:::tip
Every numeric operation — filters, key conditions, `incrementValue`/`decrementValue` — accepts `bigint` anywhere it accepts `number`. Use it for values outside JavaScript's safe integer range; DynamoDB's own `Number` type has no such limit.
:::
